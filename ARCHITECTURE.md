...
| [docs/DOOR-ASSISTANT.md](docs/DOOR-ASSISTANT.md) · [docs/DOOR-CODE.md](docs/DOOR-CODE.md) | **What each door actually is today, room by room (2026-08-05).** Written as a record of the shipped state, with what is missing marked as missing and the "what should it also do" line left blank for the owner. COMPANY.md is the shape; these two are the inventory. |
...
|---|---|---|
| 1. Model management | Real behavior, but **not its own module** — lives inside `internal/model` (interface + all 14 provider clients + factory/bootstrap) and `internal/provider` (catalog), both part of the same flat root module. `providers/` (scaffold, §4) is where this is *meant* to move — zero code there yet. |
...
| `model` | 14 | `Provider` interface, `Message`/`Request`/`Response` types, factory, bootstrap, and **all 14 provider client implementations** in the same package. Imports `internal/provider` (see [§6.2](#62-modelprovider-imports-providers-catalog)). | [README](internal/model/README.md) |
...
| `mode` | 2 | Session modes (§83/§84) — one desk per kind of work. Bundled `modes/*.md` (assistant · coding · specialized) shadowed by `<DataRoot>/modes/*.md`; `categories:`/`tools:`/`deny:` select built-ins, default-closed `mcp:` attaches servers, default-closed `dispatch:` names the desks this one may hand a job to, the body is the direction prompt. `Carries` is the one place a registered tool is judged (MCP by server, skills always kept); `Office` is the desk chairs sit at. Not an identity (§44.0) and not a permission tier. | §83, §84 |
