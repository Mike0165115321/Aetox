...
Five changes shipped together, all pointed at the same claim: a cheap model with a small window is enough. Four make context cheaper; the fifth makes the §67 store answer questions. A study of Hermes Agent (`docs/hermes-agent-teardown.md`, local-only) supplied the patterns; each was taken only where it already fit an Aetox decision, not because Hermes has it.
...
**Not taken from the study, on purpose.** Messaging platforms, cloud terminal backends, a model-subscription portal, computer use — each fails the one-sentence test ("does it help an ordinary person finish repetitive work on their own machine faster?") or contradicts the local-data line. The MoA loop stays unread until ADR 0002 is actually decided, as prior art on what an ensemble costs per answer.
