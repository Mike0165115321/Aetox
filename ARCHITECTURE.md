...
| `mode` | 2 | Session modes (§83) — one desk per kind of work. Bundled `modes/*.md` (assistant · coding · specialized) shadowed by `<DataRoot>/modes/*.md`; `categories:`/`tools:`/`deny:` select built-ins, default-closed `mcp:` attaches servers, the body is the direction prompt. Not an identity (§44.0) and not a permission tier. | §83 |
...
- **A construct it cannot read stops the command** rather than passing through: `$(…)`, backticks, `${…}`, `-EncodedCommand`, `Invoke-Expression`, and a variable in path position that is not one of the folder-naming names. This is where it parts from OpenCode, whose shell tool is the better-engineered version of the idea (real tree-sitter grammars for bash and PowerShell, provider paths, cygpath) but checks paths only for a known verb list (`FILES`) and *skips* tokens whose value is dynamic. Both are quiet holes — `findstr`, `python -c` and `Get-Content` are on nobody's verb list, and a token skipped for being hard to read is a permission granted by the parser's limits.
